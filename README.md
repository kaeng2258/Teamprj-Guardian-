...
### 2. 🤝 실시간 케어 & 커뮤니케이션
- **역할별 워크스페이스**: 환자(Client), 매니저(Manager) 각 역할에 최적화된 화면 제공
...
- 황정성: 백엔드 전반(아키텍처, DB 모델링, 인증/JWT, 매칭/복약/알림/WebPush) 설계·구현 + 프론트엔드 전반(SPA 구조, 페이지/UX/로직) 개발, 인프라 최적화

- 서수한: e약은요 외부 API연결, 실시간 채팅 구현, 배포 자동화
...
## 🧭 향후 개선 사항
- 화상채팅 안정화(TURN 서버 추가 및 네트워크 환경별 ICE 튜닝)
- Web Push 토큰 만료/재등록 자동화 및 알림 재시도 정책 개선
- 매칭 히스토리/담당자 변경 이력 조회 화면 추가
- 채팅/알림 로그 아카이빙 및 관리자 모니터링 대시보드
- 다국어(i18n) 지원 및 접근성(ARIA/키보드 네비게이션) 강화
